...
for i in range(count):
    ...
    if circuitsdata[i][9] == "a":
        renderPDF.draw(scaleSVG("SVG/Counterclockwise_arrow.svg", clockwisescale), my_canvas, circuit_x + left_margin + 8 + int(circuitsdata[i][10]), circuit_y + bottom_margin + int(circuitsdata[i][11]) - 5)
    else:
        renderPDF.draw(scaleSVG("SVG/Clockwise_arrow.svg", clockwisescale), my_canvas, circuit_x + left_margin + 8 + int(circuitsdata[i][10]), circuit_y + bottom_margin + int(circuitsdata[i][11]) - 5)
